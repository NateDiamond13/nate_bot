...
pub async fn get_video_details(url_or_search: &str, data: &CommandData) -> Result<VideoDetails> {
    ...
    let url = if is_valid_url(&url_or_search) {
        url_or_search.clone()
    } else {
        get_url_by_search(&url_or_search).await?
    };
...
pub async fn _get_video_details(url_or_search: &str, data: &CommandData) -> Result<VideoDetails> {
    let url_or_search = url_or_search.trim().to_string();
    if is_valid_url(&url_or_search) {
        if !_is_valid_youtube_url(&url_or_search) {
            // If it's a valid YouTube url, find it directly with rusty_ytdl
            _get_youtube_video_by_url(&url_or_search, data).await
        } else {
...
async fn _get_youtube_video_by_url(url: &str, data: &CommandData) -> Result<VideoDetails> {
...
fn _is_valid_youtube_url(url: &str) -> bool {
